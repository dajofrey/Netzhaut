...
NH_BEGIN()
    ...
    if (Node_p->text_p != NULL) {NH_CHECK(Nh_Vk_updateText(Tab_p, Node_p))}

    if (NH_HTML_SHOW_IMAGE(Node_p)) {NH_CHECK(Nh_Vk_updateImageVertices(Tab_p, &GPU_p->Driver, Node_p, 0.0f))}
    else                            {NH_CHECK(Nh_Vk_updateBackgroundVertices(Tab_p, &GPU_p->Driver, Node_p))}

    for (int i = 0; i < Node_p->Computed.Properties.Background.Images.count; ++i) {
        NH_CSS_Image *Image_p = Nh_getListItem(&Node_p->Computed.Properties.Background.Images, i);
        NH_CHECK(Nh_Vk_updateBackgroundImageVertices(Tab_p, &GPU_p->Driver, Node_p, Image_p))
    }

    if (Node_p->Computed.Properties.Position.display == NH_CSS_DISPLAY_LIST_ITEM) {}

    if (NH_CSS_SHOW_TOP_BORDER(Node_p))    {NH_CHECK(Nh_Vk_updateBorderVertices(Tab_p, &GPU_p->Driver, Node_p, NH_VK_BUFFER_TOP_BORDER_VERTICES))}
    if (NH_CSS_SHOW_RIGHT_BORDER(Node_p))  {NH_CHECK(Nh_Vk_updateBorderVertices(Tab_p, &GPU_p->Driver, Node_p, NH_VK_BUFFER_RIGHT_BORDER_VERTICES))}
    if (NH_CSS_SHOW_BOTTOM_BORDER(Node_p)) {NH_CHECK(Nh_Vk_updateBorderVertices(Tab_p, &GPU_p->Driver, Node_p, NH_VK_BUFFER_BOTTOM_BORDER_VERTICES))}
    if (NH_CSS_SHOW_LEFT_BORDER(Node_p))   {NH_CHECK(Nh_Vk_updateBorderVertices(Tab_p, &GPU_p->Driver, Node_p, NH_VK_BUFFER_LEFT_BORDER_VERTICES))}
