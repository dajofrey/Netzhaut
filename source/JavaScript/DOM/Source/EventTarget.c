...
NH_BEGIN()
    ...
    if (aCount == 2) // set
    {
        ...
        if (hashmap_get(NH_HASHMAPS.JS.EventTypes, Arguments_p[0].data_p, (void**)(&HashValue_p)) != MAP_OK) {NH_END(Nh_JS_getNULLResult())}
...
NH_BEGIN()
    ...
    if (aCount == 1 && Arguments_p->type == NH_JS_TYPE_OBJECT)
    {
        ...
        Event_p->Target.data_p     = Element_p;        
        Event_p->Target.distinct_p = Nh_JS_getDistinction(Script_p, Event_p->Target.data_p);
