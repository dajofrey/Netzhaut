...
NH_BEGIN()
    ...
    NH_CHECK(Nh_JS_createObject(Script_p, NH_JS_OBJECT_MOUSE_EVENT, Object_pp))
    (*Object_pp)->data_p = Nh_allocate(sizeof(NH_JS_MouseEvent));
    NH_CHECK_MEM((*Object_pp)->data_p);
    *((NH_JS_MouseEvent*)(*Object_pp)->data_p) = *MouseEvent_p;
