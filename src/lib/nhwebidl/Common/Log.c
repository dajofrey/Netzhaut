...
NH_WEBIDL_BEGIN()
    ...
    for (int i = 0; i < Tokens_p->length; ++i) 
    {
        ...
        sprintf(node_p, dirty ? "nhwebidl:Parser:%s:TokensDirty" : "nhwebidl:Parser:%s:TokensClean", fragmentName_p);
...
NH_WEBIDL_BEGIN()
    ...
    sprintf(node_p, "nhwebidl:Parser:%s:ParseTree", fragmentName_p);
    nh_core_sendLogMessage(node_p, NULL, message_p);
...
NH_WEBIDL_BEGIN()
    ...
    for (int i = 0; i < Fragment_p->Interfaces.length; ++i) 
    {
        ...
        NH_BYTE node_p[255] = {0};
        sprintf(node_p, "nhwebidl:%s:Interfaces:%s", specification_p, className_p);
