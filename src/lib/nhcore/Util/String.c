    ...
    NH_BYTE *bytes_p)
{
    ...
    NH_BYTE *allocatedBytes_p = nh_core_allocate(sizeof(NH_BYTE) * (strlen(bytes_p) + 1));
...
{
NH_CORE_BEGIN()
NH_CORE_END(nh_core_initArray(sizeof(NH_BYTE), chunkSize))
}
...
NH_CORE_BEGIN()
    ...
    if (format_p == NULL) {NH_CORE_DIAGNOSTIC_END(NH_CORE_SUCCESS)}

    va_list args;
    va_start(args, format_p);

    int byteCount = vsnprintf(NULL, 0, format_p, args);
    NH_BYTE *append_p = nh_core_allocate(sizeof(NH_BYTE) * (byteCount + 1));
    ...
    NH_CORE_CHECK(nh_core_appendToArray(String_p, append_p, byteCount * sizeof(NH_BYTE)))
