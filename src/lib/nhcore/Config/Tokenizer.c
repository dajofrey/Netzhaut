...
NH_CORE_BEGIN()
    ...
    switch (*tokenBegin_p) 
    {
        ...
        case '-' : Token_p->type = NH_CONFIG_TOKEN_HYPHEN_MINUS; break;
    ...
    while (*bytes_p && (nh_core_isASCIIAlpha(*bytes_p) || nh_core_isASCIINumber(*bytes_p) || *bytes_p == '_' || *bytes_p == '.')) {bytes_p++;}
    if (!*bytes_p) {NH_CORE_END(NULL)}
