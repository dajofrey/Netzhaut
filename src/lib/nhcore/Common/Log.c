...
NH_CORE_BEGIN()
    ...
    NH_BYTE id_p[127] = {'\0'};
    sprintf(id_p, "nhcore:Threads:%p", Thread_p);
...
NH_CORE_BEGIN()
    ...
    for (int i = 0; i < NH_MODULE_E_COUNT; ++i) 
    {
        ...
        if (!NH_LOADER.Modules_p[i].lib_p) {
            ...
            switch (i) {
                ...
                case NH_MODULE_MAKE :
                    ...
                    break;
                case NH_MODULE_DOCS :
                    ver_p = NH_LOADER.loadSymbol_f(i, NH_LOADER.Modules_p[i].major, "NH_DOCS_VERSION_P");
                    break;
            }
        ...
        nh_core_sendLogMessage("nhcore:Modules", option_p, message_p);
...
NH_CORE_BEGIN()
    ...
    nh_core_sendLogMessage("nhcore:System", option_p, info_p);
