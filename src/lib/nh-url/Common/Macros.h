...
{                                                                           \
    NH_URL_RESULT checkResult = checkable;                                  \
    if (checkResult != NH_URL_SUCCESS) {NH_URL_DIAGNOSTIC_END(checkResult)} \
} 

#define  NH_URL_CHECK_2(checkReturn, checkable)                  \
{                                                                \
    NH_URL_RESULT checkResult = checkable;                       \
    if (checkResult != NH_URL_SUCCESS) {NH_URL_END(checkReturn)} \
} 

#define NH_URL_SILENT_CHECK(checkable)                       \
{                                                            \
    NH_URL_RESULT checkResult = checkable;                   \
    if (checkResult != NH_URL_SUCCESS) {NH_URL_SILENT_END()} \
} 

#define NH_URL_CHECK_NULL(checkable)                                              \
{                                                                                 \
    void *checkResult_p = checkable;                                              \
    if (checkResult_p == NULL) {NH_URL_DIAGNOSTIC_END(NH_URL_ERROR_NULL_POINTER)} \
} 
...
{                                                                                      \
    void *checkResult_p = checkable;                                                   \
    if (checkResult_p == NULL) {NH_URL_DIAGNOSTIC_END(NH_URL_ERROR_MEMORY_ALLOCATION)} \
} 
