...
NH_ENCODING_BEGIN()
    ...
    if (strlen(str1_p) != strlen(str2_p)) {NH_ENCODING_END(false)}
...
NH_ENCODING_BEGIN()
    ...
    if (!p1 && !p2) {NH_ENCODING_END(true)}
    if (!p1 || !p2) {NH_ENCODING_END(false)}
...
NH_ENCODING_BEGIN()
    ...
    if (!utf32_p && !ascii_p) {NH_ENCODING_END(true)}
    if (!utf32_p || !ascii_p) {NH_ENCODING_END(false)}
