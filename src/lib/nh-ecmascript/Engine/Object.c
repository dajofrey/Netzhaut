...
static nh_ecmascript_Property* nh_ecmascript_newProperty(
    bool isAccessor)
{
    nh_ecmascript_Property *prop = nh_core_allocate(sizeof(nh_ecmascript_Property));
    if (!prop) return NULL;
    ...
    if (isAccessor) {
        ...
        prop->data.value = nh_ecmascript_undefined();
...
bool nh_ecmascript_setFunctionLength(nh_ecmascript_Object *F_p, int length) 
{
    ...
    return nh_ecmascript_createDataProperty(
        F_p, 
        "length", 
...
bool nh_ecmascript_setFunctionName(
    ...
    const char *prefix) 
{
    // 1. Assert: F_p is an extensible object with no "name" property.
    // 2. If name is a Symbol, get its description. (Simplified here for strings)
    
    char *finalName = NULL;
    if (prefix != NULL) {
        // Handle prefixes for getters/setters (e.g., "get " + "prop")
        finalName = nh_core_stringPrintf("%s %s", prefix, name.p.string);
    } else {
        finalName = nh_core_stringCopy(name.p.string);
    }

    // 3. Set the "name" property
    // Attributes: { [[Value]]: name, [[Writable]]: false, [[Enumerable]]: false, [[Configurable]]: true }
    bool success = nh_ecmascript_createDataProperty(
        F_p, "name", nh_ecmascript_makeString(finalName), false, false, true
    );

    nh_core_free(finalName);
    return success;
}
    ...
    int initialCapacity) 
{
    ...
    nh_core_initializeHashMap(&obj_p->Properties, capacity);
    ...
    nh_ecmascript_Realm *realm)
{
    ...
    F->environment = env; // This is the 'closure' capture
    F->realm = realm;
...
nh_ecmascript_Object *nh_ecmascript_createBuiltinFunction(
    ...
    nh_ecmascript_Object *prototype)
{
    // 1. Determine prototype
    nh_ecmascript_Object *proto = (prototype != NULL) ? prototype : Realm_p->intrinsics.functionPrototype;

    // 2. Create the base object
    nh_ecmascript_Object *func = nh_ecmascript_ordinaryObjectCreate(proto, NULL, 4);

    // 3. Set Internal Slots
    func->type = NH_ECMASCRIPT_OBJECT_FUNCTION_BUILTIN;
    func->nativeHandler = handler;
    func->realm = Realm_p;

    // 4. Set "length" using the standard helper (Spec 10.2.8)
    nh_ecmascript_setFunctionLength(func, length);

    // 5. Set "name" using the standard helper (Spec 10.2.9)
    nh_ecmascript_setFunctionName(func, nh_ecmascript_makeString(name), NULL);
