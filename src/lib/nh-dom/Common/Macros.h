...
{                                                                           \
    ...
    if (checkResult != NH_API_SUCCESS) {NH_DOM_DIAGNOSTIC_END(checkResult)} \
}

#define NH_DOM_CHECK_2(checkReturn, checkable)                   \
{                                                                \
    NH_API_RESULT checkResult = checkable;                       \
    if (checkResult != NH_API_SUCCESS) {NH_DOM_END(checkReturn)} \
}

#define NH_DOM_CHECK_NULL(checkable)                                              \
{                                                                                 \
    void *checkResult_p = checkable;                                              \
    if (checkResult_p == NULL) {NH_DOM_DIAGNOSTIC_END(NH_API_ERROR_NULL_POINTER)} \
}
...
{                                                                                      \
    void *checkResult_p = checkable;                                                   \
    if (checkResult_p == NULL) {NH_DOM_DIAGNOSTIC_END(NH_API_ERROR_MEMORY_ALLOCATION)} \
}
