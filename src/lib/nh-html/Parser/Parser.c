...
NH_HTML_BEGIN()
    ...
    NH_HTML_CHECK(nh_html_checkInputStream(&Parser, InputStream))
    ...
    while (!Parser.stop) {
        NH_HTML_CHECK(nh_html_consumeNext(&Tokenizer))
    }

    if (logId_p) {
        NH_HTML_CHECK(nh_html_logDocument(logId_p, Parser.Document_p))
    }
