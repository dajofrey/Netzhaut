        ...
        FT_Library *library)
{
    ...
    if (FT_HAS_FIXED_SIZES(self->ft_face)){
    	FT_Select_Size(self->ft_face, 0); //just use the first one
    } else {
        error = FT_Set_Char_Size(self->ft_face, (int)(self->size*64), (int)(self->size*64), 0, 0);
    }
...
// ------------------------------------------------------ texture_font_init ---
static int
texture_font_init(FT_Library *library, texture_font_t *self)
{
    assert(self->atlas);
    assert(self->size > 0);
    ...
    FT_Size_Metrics metrics = self->ft_face->size->metrics;
...
texture_font_t *
texture_font_new_from_file(FT_Library *library, texture_atlas_t *atlas, 
        const float pt_size, const char *filename)
{
    texture_font_t *self;
    ...
    if (texture_font_init(library, self)) {
...
texture_font_t *
texture_font_new_from_memory(FT_Library *library, texture_atlas_t *atlas, 
...
size_t
texture_font_load_glyphs( FT_Library *library,
                          texture_font_t * self,
                          uint32_t * codepoints,
                          unsigned int length,
                          const char *language )
{
    ...
    /* Layout the text */
    hb_buffer_add_utf32( buffer, codepoints, length, 0, length );
